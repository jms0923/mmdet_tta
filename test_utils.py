...
class PostProcessor():
    ...
    def saveIitp(self, imgPath, result):
        bboxes, labels = self.extractInfo(result)
    ...
    def saveScorechecker(self, file_path, result, cascadecsv):
        bboxes, labels = self.extractInfo(result)
        _, labels = self.iitpProcess(bboxes, labels)
        output_class = [0] * 8
        cascadecsv.write(file_path.split("/")[-1])
        for label in labels:
            output_class[label] = 1
        output_class.pop(0)
        for i in output_class:
            cascadecsv.write("," + str(i))
        cascadecsv.write("," + "\n")
